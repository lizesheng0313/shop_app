            ...
            <View className='hd'>
              {
                this.state.secondList.map((item, index => {
                  return <View className="second_type" onClick={this.handleSetSecond.bind(this,item,index)} className={`item ${this.state.currentActve === index ? 'active' : ''}`}>{item.title}</View>
                }))
              }
            </View>
            <View className='bd'>
